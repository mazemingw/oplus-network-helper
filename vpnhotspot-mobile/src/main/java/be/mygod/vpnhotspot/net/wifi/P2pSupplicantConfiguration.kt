...
import be.mygod.vpnhotspot.CrashReporter
...
class P2pSupplicantConfiguration(private val group: WifiP2pGroup? = null) {
    ...
    suspend fun init(ownerAddress: String? = null) {
        ...
        try {
            ...
            // 记录关键信息（无 Firebase 环境下不应导致再次崩溃）
            Timber.w(e, "P2P config parse failed. ownerAddress=%s p2pGroup=%s", ownerAddress, group)
            Timber.w("P2P raw config:\n%s", config)

            // 如果你未来要接回 Firebase，这里也可以通过 CrashReporter.enabled 再上报
